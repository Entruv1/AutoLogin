...
| | 旧版 `auto_login` | 本版 `auto_login_vision` |
|---|---|---|
| 浏览器 | 内置 Playwright Chromium | **不内置**，用你自己的浏览器 |
| 驱动方式 | DOM 选择器 + CDP 协议 | **纯视觉**：只截屏、只动鼠标键盘 |
| 页面改版 | 选择器失效就挂 | 只要人眼还能认出按钮就不受影响 |
| 指纹 | 带自动化特征 | 与真人操作无区别 |
| 体积 | 内置 Chromium，很大 | 只有一个 exe + 40MB 识别模型 |
...
这几个是纯视觉方案在真实浏览器里才会遇到的问题，也是本版和早期版本的主要差别：

| 现象 | 原因 | 处理 |
|---|---|---|
| **Edge 的「保存的密码」下拉挡框** | 它是**浏览器自己的 UI**：弹在输入框正下方，盖住密码框、单位框甚至登录按钮；页面里的点击落上去就等于点了那条凭据，而且它不一定吃 `ESC` | 账号框点一下聚焦，之后**改用 Tab 移动焦点**绕开它（见上文第 5 点）；焦点一移开下拉就自己收起来 |
| **密码粘进了用户名框** | Bitwarden 等密码管理器的自动填充下拉弹在输入框**正下方**，盖住了下一个输入框；点击落在下拉上，粘贴就跟着进了上一个框 | 点击输入框前先 `ESC`、再点卡片外的空白让它失焦；**点完立刻用聚焦描边复核焦点**，不在目标框就重试；填完再视觉自证一遍，发现空框就重填 |
| **输入法候选窗也会挡框** | 微软拼音的候选词窗口（"1 想 2 县 3 下…"）同样是浮层，而且**会吃掉 Tab** | 焦点不在三个框上时（说明被浮层占着）才补按 `ESC` 清场；注意**不能无条件先按 ESC** —— 实测"ESC 后再 Tab"反而会让 Tab 失效 |
| **点了登录却毫无反应** | 单位输入框还带着焦点，第一次点击被拿去关闭自动补全下拉了 | 等待弹窗时若页面 1.6 秒内毫无帧差，自动补点一次登录（最多 2 次） |
| **被别的窗口盖住却报"登录成功"** | 整屏帧差占比很大 + 找不到登录表单 ≈ 页面已跳转，但"被别的窗口挡住"也是这个特征 | 判定跳转前先按标题把目标浏览器窗口拉回前台再复核 |
| **命令行开的 Edge 抢不到前台** | `SetForegroundWindow` 受"前台锁定"限制，从后台进程发起的启动常被系统压回后面 —— 窗口开了却还盖在别的窗口底下，探测一直失败、白等 21 秒才超时 | 打开之后主动补一刀：轮询找到浏览器窗口并 `SetForegroundWindow`（`PageOpener._raise_browser`） |
| **`Ctrl+L` 改地址把用户的标签页顶掉** | 早期版本为了"导航到登录页"直接 `Ctrl+L` + 粘地址 + 回车 —— 变的是**当前标签页**，用户正在看的页面就没了 | 改成命令行 `msedge.exe <url>`（落新标签页）或 `Ctrl+T`，只增不覆盖 |
