...
- **导出 / 导入配置**：设置界面里能把配置整体导出成文件、也能从文件导入，换机 / 备份不用重敲；
...
| 按钮 | 行为 |
|---|---|
| **导出到文件…** | 把当前**全部**设置写成一个 JSON 文件（默认名 `autologin-config-年月日.json`） |
| **从文件导入…** | 读一个 JSON 文件，整份覆盖当前设置 |

几个细节：

- **导出带的是界面上当前的样子，不是上次保存的样子** —— 填了一半还没点保存也会一起导出，
  不会出现"导出来的比屏幕上少"的情况。
- **格式和 `config.json` 完全一致**（同一段解析代码），所以：
  导出文件可以直接改名成 `config.json` 用；反过来 `config.json` 也能直接当导入文件喂进去。
- **导入前会先确认**，确认框里把「将导入什么」和「将替换什么」都列出来（站点 + 账号数，
  以及有几个账号是填了密码的），点错也有得反悔。
- **确认后先备份再覆盖**：旧设置会写成 `config.json.bak`（就在 `config.json` 旁边），
  然后才落盘并**重新注册热键**（导入进来的热键立刻生效，不用重启）。
- **文件有问题就什么都不动**：不是合法 JSON、`accounts` 不是列表、`retries` 不是数字
  等等，都会弹框说清楚是哪一行出的问题，当前配置保持原样。

> 导出的文件里含账号和密码（base64 混淆 ≈ 明文），别随手发给别人。
...
├── config.json.bak   导入配置时自动生成的旧配置备份（**已 gitignore**）
├── .gitignore
├── run.bat
└── build.bat
```

> `config.json`、`config.json.bak`、`_debug/`、`_ref/`、`dist/`、`build/` 都在 `.gitignore` 里 ——
> 真实账号、站点截图和调试日志不会被提交。
